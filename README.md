...
- Hold breath recovers correctly from aim fatigue, a new **Player Stamina** slider controls sprint, breath-hold, and jump endurance, and an optional stamina bar makes the remaining native stamina visible while on foot.
- Player and AI handheld weapons recover from stale vehicle-seat state instead of remaining unable to fire on foot.

## What's new in 1.1.0

- Infantry posture and movement ownership were reworked to stop crouch/stand, crouch/prone, run/stop, inch-forward, and aim/lower loops.
- Attackers now use five-second crouched bounds under fire and configurable 5–15 second firing holds, while the commander reassesses every 24 seconds without interrupting unchanged movement.
- Cover slots remain exclusive through brief native-state flicker, and overlapping soldiers separate instead of sharing one position.
- AI can reacquire recent shooters concealed by vegetation without seeing through solid cover, and confirmed sight loss now requires fresh visual reacquisition.
- Added **AT Launcher Accuracy**, renamed **Awareness** to **Perception**, and preserved vehicle reboarding after AI repairs.
...
- **Unsupported aiming develops recoverable native weapon sway.** Crouching lasts longer before fatigue sets in, while prone aiming or held breath provides support. The separate **Player Stamina** slider scales native sprint, breath-hold, and jump endurance from 0.5x to 3x, with 1x preserving the base game. The optional **Stamina Bar** shows the local player's remaining native stamina at the bottom right while alive and on foot, and is disabled by default.
...
- **Ground turrets combine third-person point aim with separate controls for the unzoomed gunner view and optical gunsight.** In third-person view, mouse or stick input independently moves the active gunner camera while the selected tank turret or direct-fire static weapon follows its center-screen direction at native traverse and elevation rates. Exterior camera and bore sight lines remain parallel; a thin translucent gray large circle marks the commanded camera aim, and a smaller matching circle shows the visible true bore direction. The separate Show Ground Vehicle Aim Rings option hides both circles without changing the controls or aircraft indicators. In a vehicle's unzoomed first-person gunner/periscope view, the enabled-by-default Lock Gunner View Elevation option permits horizontal traversal but holds the gun's local elevation, preventing that view from vertically point-aiming the cannon or coaxial machine gun while hull motion still moves the bore naturally. In the right-click optical gunsight, the separate enabled-by-default Unstabilized Gunsight option applies relative input directly to the weapon and attaches the view to the physical gun, preventing it from compensating vertically to preserve a world aim point as the vehicle moves. Each option can be disabled independently. The horizon stays level; commander, driver, AI, and remote-player controls remain native. Native turret limits, damage slowdown, firing, synchronization, and adjustable range zeroing remain in use.
...
- **Aircraft can use the same third-person point-aim model on mouse and keyboard.** The separate Mouse Point Aiming switch makes the large white circle the unrestricted camera target and the smaller white circle the selected weapon's native zeroed boresight, falling back to aircraft nose only when weapon data is unavailable. The large circle can reach every screen edge and corner while a firm, smooth chase camera follows it without snapping; mouse movement stays aligned with the displayed screen axes after banking or inversion. The camera can orbit completely around the aircraft. Flight guidance uses its approximately 178-degree forward hemisphere around the aircraft's reliable direction of travel: crossing behind parks the flight command at the last legal boundary, and further rearward camera movement cannot steer the aircraft or reconnect automatically as it turns. Fresh mouse movement reconnects point guidance only after the view returns to the forward hemisphere. Fine left/right corrections coordinate rudder while leveling existing bank; larger errors blend into automatic bank. Straight-ahead downward aim can request the full forward-stick range before the physical model applies its deliberately weaker nose-down response, while lateral dives bank and pull. **A/D** provide pure manual roll with neutral elevator while held, then return roll to the instructor. Pitch remains tied to the commanded direction instead of changing merely because the aircraft is banked. Holding the vehicle-look action (**C** by default) freezes the flight command while the camera looks freely. The same configurable **C** + **Shift** chord smoothly applies the separate Aircraft Free Look Zoom in both first- and third-person aircraft views; it defaults to 4x, and releasing either key smoothly restores the ordinary field of view. Releasing vehicle look smoothly returns the camera to the circle without a visible final jump. First-person aircraft controls and camera movement remain native apart from this optical zoom; bomb-sight views, AI, and remote aircraft remain unchanged. Simplified mouse-and-keyboard controls gain an independent enabled-by-default Manual Roll option: the configured aircraft-roll actions (A/D by default) command axial roll while held, then hand control back to native Simplified auto-leveling. Neither aircraft option intercepts physical-controller input, and the existing controller-freelook steering option remains separate. Tank controllers continue to use the same point-aim scheme as mouse input.
...
- **Spectator view can retain the full gameplay HUD.** An enabled-by-default Player Experience option keeps native squad and unit markers, squad and player status, objectives, and the battle phase/progress bar visible in the cinematic spectator camera while respecting the game's individual HUD preferences. Press **F** by default to toggle every spectator HUD element together.
...
Quick Setup and the AI / Core Behavior page also expose centered 0.5x-1.5x sliders for Aggressiveness, Accuracy, Reaction Speed, Perception, and Suppression Resistance; 1.0x is the recommended baseline. Perception coordinates field of view, peripheral awareness, contact memory, and the distance at which an identified enemy stops an advance; it never grants line of sight. Detailed AI options remain available under Objectives, Attack Bonuses, Defense, Infantry Tactics, Vehicle Tactics, Support, and Diagnostics. Defender static-weapon staffing has its own switch on the Defense page. Non-AI settings are unchanged.
...
The split ground-gunner control scheme is now implemented: third-person view uses point aiming, the unzoomed first-person gunner/periscope view can lock local gun elevation, and the optical gunsight defaults to direct unstabilized control. Both first-person behaviors have separate toggles, and adjustable range zeroing is retained. Aircraft mouse point aiming applies only to the local player's third-person mouse-and-keyboard Realistic controls, while a separate keyboard-only option adds bound-key manual roll to Simplified controls. Neither aircraft option intercepts physical-controller input. AI and remote players remain on the native controllers, while ground-turret rotation and aircraft response continue through the game's native movement and synchronization paths. The next step is to runtime-validate these aiming controls and continue the read-only audit of Easy Red 2's existing whole-vehicle, engine, hull, fuel-tank, track, wheel, penetration, repair, destruction, and networked damage paths before proposing any damage adjustment.
...
The physical flight model is local to the human aircraft personally controlled by each player running the mod. Easy Red 2's native vehicle synchronizer carries that aircraft's resulting motion to the room. Unmodded pilots, aircraft controlled by other players, and all AI aircraft retain native flight physics.
