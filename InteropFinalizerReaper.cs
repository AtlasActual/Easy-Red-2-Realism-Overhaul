...
///
/// This reaper suppresses finalization only for base-game wrappers generated in
/// Assembly-CSharp. UnityEngine wrappers and types injected by this or another
/// plugin retain Il2CppInterop's stock lifetime behavior. When Il2CppObjectPool
/// publishes its own WeakReference for an eligible wrapper, the reaper reuses that
/// existing liveness handle; only eligible wrappers that bypass the pool receive a
/// private non-finalizable weak GCHandle. A below-normal-priority worker releases
/// dead native handles in bounded batches, atomically detaches stale pool entries,
/// and then retires their managed weak handles after a grace period for concurrent
/// readers.
/// </summary>
internal static class InteropFinalizerReaper
{
    ...
    private static readonly Assembly GameInteropAssembly = typeof(Soldier).Assembly;
    ...
    internal static bool TryInstall(Harmony harmony)
    {
        ...
        try
        {
            ...
            Plugin.LogSource.LogInfo(
                "Deferred IL2CPP wrapper cleanup installed for base-game " +
                "Assembly-CSharp wrappers only. UnityEngine and plugin-injected " +
                "wrapper types retain Il2CppInterop's stock lifetime behavior.");
            return true;
        }
        catch (Exception ex)
        {
    ...
    private static void OnWrapperConstructed(Il2CppObjectBase __instance)
    {
        ...
        try
        {
            if (!IsBaseGameWrapper(__instance))
            {
                return;
            }
    ...
    private static bool IsBaseGameWrapper(Il2CppObjectBase wrapper)
    {
        // Il2CppObjectPool is process-wide, so there is no reliable "creating
        // plugin" owner to inspect. Runtime wrapper type is the stable boundary:
        // generated game types live in Assembly-CSharp, injected MonoBehaviours
        // live in their plugin assembly, and Unity objects live in UnityEngine
        // interop assemblies. Strong references held by any plugin still keep an
        // eligible shared wrapper alive and prevent its native handle being reaped.
        return ReferenceEquals(wrapper.GetType().Assembly, GameInteropAssembly);
    }
