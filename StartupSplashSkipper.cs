...
internal static class StartupSplashSkipper
{
    ...
    internal static void SkipGameSplash(LoadingScreen2 loadingScreen)
    {
        try
        {
            // Easy Red 2 has a second, game-authored splash sequence in its first loading scene.
            // Preserve LoadingScreen2 itself because its coroutine performs the real boot-time asset loads.
            ...
            if (!_gameSplashSkipped)
            {
                ...
                Plugin.LogSource.LogInfo("Skipped Easy Red 2's startup logo animation while retaining its asset loading sequence.");
            }
        }
        catch (Exception exception)
        {
...
internal static class LoadingScreenSplashPatches
{
    ...
    {
        StartupSplashSkipper.SkipGameSplash(__instance);
    }
