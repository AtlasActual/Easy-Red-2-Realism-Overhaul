...
internal static class Program
{
    private static int Main()
    {
        var tests = new (string Name, Action Run)[]
        {
            ...
            (nameof(CloseContactSeeksBlindProtectionAfterFiringPhase), CloseContactSeeksBlindProtectionAfterFiringPhase),
            (nameof(CloseCoverSearchKeepsSafetyAndRetryGates), CloseCoverSearchKeepsSafetyAndRetryGates),
            (nameof(ContinuousCloseContactDoesNotRestartFiringDeadline), ContinuousCloseContactDoesNotRestartFiringDeadline),
            ...
            (nameof(CoverSelectionScoresEstimatedRouteInsteadOfVetoingEntrances), CoverSelectionScoresEstimatedRouteInsteadOfVetoingEntrances),
            (nameof(DefensiveAreaEntryDoesNotOwnAnEmptyCoverAssignment), DefensiveAreaEntryDoesNotOwnAnEmptyCoverAssignment),
            (nameof(ProtectedUrbanCoverDoesNotRequireDistantLineOfSight), ProtectedUrbanCoverDoesNotRequireDistantLineOfSight),
            (nameof(DefensiveVisibilityPreferenceDoesNotOverruleBetterProtection), DefensiveVisibilityPreferenceDoesNotOverruleBetterProtection),
            (nameof(DefensiveUpgradesRequireMeasuredProtectionGain), DefensiveUpgradesRequireMeasuredProtectionGain),
            (nameof(DefensiveUpgradesStayLocalAndAvoidExposedRoutes), DefensiveUpgradesStayLocalAndAvoidExposedRoutes),
            (nameof(DefensiveReassessmentWaitsForQuietAndCooldown), DefensiveReassessmentWaitsForQuietAndCooldown),
            (nameof(CloseContactCannotPermanentlyVetoCommittedUrbanMovement), CloseContactCannotPermanentlyVetoCommittedUrbanMovement),
            ...
            (nameof(DefensivePositionOwnershipYieldsToNativeVehicleBoarding), DefensivePositionOwnershipYieldsToNativeVehicleBoarding),
            ...
            (nameof(HaltSpacingOnlyProtectsAnActuallyOccupiedReservation), HaltSpacingOnlyProtectsAnActuallyOccupiedReservation),
            (nameof(HaltSpacingResolvesTwoOverlappingSlotOwners), HaltSpacingResolvesTwoOverlappingSlotOwners),
            (nameof(HaltSpacingCanExitAnOverlappingBodyButNotCrossOthers), HaltSpacingCanExitAnOverlappingBodyButNotCrossOthers),
    ...
    private static void AttackCoverCorridorAllowsFlankingWithinBoundedBacktrack()
    {
        ...
        // A nearby side/rear doorway remains usable within the backtrack allowance.
        True(AttackCoverCorridorCore.Accepts(
                soldier, new MapPoint(6f, -1f), new MapPoint(0f, 100f)),
            "A nearby rear doorway was rejected solely for its bearing.");
        True(AttackCoverCorridorCore.Accepts(
                soldier, new MapPoint(12f, 0f), new MapPoint(0f, 100f)),
            "A lateral trench entrance was rejected solely for its bearing.");
        False(AttackCoverCorridorCore.Accepts(
                soldier, new MapPoint(50f, 0f), new MapPoint(0f, 100f)),
            "A lateral detour beyond the backtrack allowance was accepted.");
        True(AttackCoverCorridorCore.Accepts(
                soldier, new MapPoint(0f, -8f), new MapPoint(0f, 100f)),
            "The exact rear backtrack boundary was rejected.");
        False(AttackCoverCorridorCore.Accepts(
                soldier, new MapPoint(0f, -8.1f), new MapPoint(0f, 100f)),
            "A rear destination beyond the backtrack boundary was accepted.");
    ...
    private static void HaltSpacingOnlyProtectsAnActuallyOccupiedReservation()
    {
        True(HaltSpacingCore.HoldsOccupiedSlot(true, false, 0.5f, 0f),
            "A reached, owned cover slot lost its protection.");
        False(HaltSpacingCore.HoldsOccupiedSlot(true, false, 3f, 0f),
            "A distant reservation blocked physical overlap correction.");
        False(HaltSpacingCore.HoldsOccupiedSlot(true, true, 0.5f, 0f),
            "A paused cover route was mistaken for an occupied slot.");
        False(HaltSpacingCore.HoldsOccupiedSlot(false, false, 0.5f, 0f),
            "An expired or stolen reservation blocked correction.");
        False(HaltSpacingCore.HoldsOccupiedSlot(true, false, 0.5f, 3f),
            "A slot on another floor blocked correction.");
        False(HaltSpacingCore.HoldsOccupiedSlot(true, false, float.NaN, 0f),
            "An invalid location was treated as occupied cover.");
    }

    private static void HaltSpacingResolvesTwoOverlappingSlotOwners()
    {
        False(HaltSpacingCore.ShouldYieldToNeighbour(true, true, true, true, 10, 20),
            "Both overlapping slot owners yielded instead of choosing one.");
        True(HaltSpacingCore.ShouldYieldToNeighbour(true, true, true, true, 20, 10),
            "Both overlapping slot owners were exempt from correction.");
        False(HaltSpacingCore.ShouldYieldToNeighbour(true, true, true, false, 20, 10),
            "Adjacent but non-overlapping cover occupants were displaced.");
        False(HaltSpacingCore.ShouldYieldToNeighbour(true, false, true, true, 20, 10),
            "The retained slot owner yielded to an unanchored AI intruder.");
        True(HaltSpacingCore.ShouldYieldToNeighbour(false, true, true, true, 10, 20),
            "An intruder did not yield to an established occupant.");
        True(HaltSpacingCore.ShouldYieldToNeighbour(true, false, false, true, 10, 20),
            "A player-controlled neighbour was expected to execute mod spacing.");
        True(HaltSpacingCore.ShouldYieldToNeighbour(true, true, true, true, -10, -20),
            "Negative Unity instance IDs broke the deterministic yield rule.");
    }

    private static void HaltSpacingCanExitAnOverlappingBodyButNotCrossOthers()
    {
        True(HaltSpacingCore.MayIgnoreDepartureBody(true, true, false),
            "A soldier's own collider blocked escape.");
        True(HaltSpacingCore.MayIgnoreDepartureBody(false, true, true),
            "The body already overlapping the departure point blocked escape.");
        False(HaltSpacingCore.MayIgnoreDepartureBody(false, true, false),
            "Spacing allowed crossing a non-overlapping friendly body.");
        False(HaltSpacingCore.MayIgnoreDepartureBody(false, false, true),
            "Spacing ignored an enemy body.");
    }
    ...
    private static void DefensiveVisibilityPreferenceDoesNotOverruleBetterProtection()
    {
        var outdoor = new CoverScoreInput(64f, 0f, true, 0, true, true, 1f, 0.1f,
            PrimaryProtectionFraction: 0.75f, PreferProtectionOverFiringLine: true);
        var sheltered = outdoor with { DistanceSqr = 256f, PrimaryProtectionFraction = 1f,
            AssignedPoseCanFire = false, StandingCanFire = false, ExposedRouteMeters = 3f };
        True(InfantryCoverDecisionCore.Score(CoverSelectionMode.DefensiveOccupation, sheltered) <
             InfantryCoverDecisionCore.Score(CoverSelectionMode.DefensiveOccupation, outdoor),
            "Substantially safer indoor cover still lost to immediate outdoor visibility.");
        var sameWithLane = sheltered with { AssignedPoseCanFire = true };
        Near(120f, InfantryCoverDecisionCore.Score(CoverSelectionMode.DefensiveOccupation, sheltered) -
                   InfantryCoverDecisionCore.Score(CoverSelectionMode.DefensiveOccupation, sameWithLane),
            0.001f, "Defensive sightline penalty was not bounded.");
        Near(500f, InfantryCoverDecisionCore.Score(CoverSelectionMode.Normal, sheltered) -
                   InfantryCoverDecisionCore.Score(CoverSelectionMode.Normal, sameWithLane),
            0.001f, "Defensive tuning changed ordinary attacker firing preferences.");
    }

    private static void DefensiveUpgradesRequireMeasuredProtectionGain()
    {
        var current = new CoverPositionQuality(true, true, 0.55f, true);
        var better = new CoverPositionQuality(true, true, 0.80f, true);
        True(InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, better, 12f, 0.2f),
            "A measured 25-point protection upgrade was rejected.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(better, current, 12f, 0.2f),
            "The upgrade policy allowed returning to the worse slot.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, current, 12f, 0.2f),
            "Equal positions caused cover churn.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current,
            better with { ProtectionFraction = 0.74f }, 12f, 0.2f), "A marginal gain triggered a move.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current,
            better with { HasFiringLane = false }, 12f, 0.2f), "Lost firing quality needed no extra gain.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current,
            better with { IsMeasured = false }, 12f, 0.2f), "An unmeasured candidate displaced an anchor.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current with { IsMeasured = false },
            better, 12f, 0.2f), "Unknown current protection was treated as inferior.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current,
            better with { IsProtective = false }, 12f, 0.2f), "An unprotected destination was accepted.");
    }

    private static void DefensiveUpgradesStayLocalAndAvoidExposedRoutes()
    {
        var current = new CoverPositionQuality(true, true, 0.5f, true);
        var better = new CoverPositionQuality(true, true, 0.8f, true);
        True(InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, better, 20f, 0.35f),
            "An upgrade at the distance/exposure boundary was rejected.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, better, 20.1f, 0.35f),
            "An optional upgrade sent a defender too far away.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, better, 2f, 0.1f),
            "A move within the same footprint was treated as an upgrade.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, better, 12f, 0.36f),
            "An optional upgrade accepted a substantially exposed estimated route.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current, better, float.NaN, 0.1f),
            "An invalid distance was accepted.");
        True(!InfantryCoverDecisionCore.IsWorthDefensiveRelocation(current,
            better with { ProtectionFraction = float.NaN }, 12f, 0.1f), "Invalid protection was accepted.");
    }

    private static void DefensiveReassessmentWaitsForQuietAndCooldown()
    {
        var next = InfantryCoverDecisionCore.NextDefensiveReassessmentAt(100f, -13);
        True(next >= 130f && next <= 140f, "Reassessment was not staggered within 30-40 seconds.");
        True(!InfantryCoverDecisionCore.ShouldReassessDefensiveCover(true, false, true, next - 1f, next, 0f),
            "An anchor was reassessed before its minimum hold.");
        True(InfantryCoverDecisionCore.ShouldReassessDefensiveCover(true, false, true, next, next, 0f),
            "A calm established anchor never became eligible.");
        True(!InfantryCoverDecisionCore.ShouldReassessDefensiveCover(true, false, false, next, next, 0f),
            "An optional upgrade interrupted combat or a required action.");
        True(!InfantryCoverDecisionCore.ShouldReassessDefensiveCover(true, true, true, next, next, 0f),
            "A committed route was reassessed during transit.");
        True(!InfantryCoverDecisionCore.ShouldReassessDefensiveCover(true, false, true, next, next, next + 1f),
            "Relocation cooldown was ignored.");
        True(!InfantryCoverDecisionCore.ShouldReassessDefensiveCover(false, false, true, next, next, 0f),
            "An unanchored soldier ran the optional upgrade policy.");
        var afterArrival = InfantryCoverDecisionCore.NextDefensiveReassessmentAt(next + 10f, -13);
        True(afterArrival >= next + 40f, "A new arrival did not receive a fresh minimum hold.");
    }
    ...
    private static void DefensivePositionOwnershipYieldsToNativeVehicleBoarding()
    {
        // Issue #4: native HoldArea sends defenders to an empty static gun as a cover.
        // A latched position that survived the boarding order froze them beside it.
        False(DefensivePositionOwnershipCore.ShouldOwn(
                new DefensivePositionOwnershipInput(
                    true, true, true, true, true, BoardingVehicle: true)),
            "An established defensive position kept halting a soldier sent to crew a gun.");
        False(DefensivePositionOwnershipCore.ShouldOwn(
                new DefensivePositionOwnershipInput(
                    false, true, true, false, false, BoardingVehicle: true)),
            "A defender walking to a gun inside the area re-acquired a stationary hold.");
        True(DefensivePositionOwnershipCore.ShouldOwn(
                new DefensivePositionOwnershipInput(
                    false, true, true, false, false, BoardingVehicle: false)),
            "A defender whose boarding order ended could not reclaim a position in the area.");
    }
    ...
    private static void CloseContactSeeksBlindProtectionAfterFiringPhase()
    {
        const float deadline = 105f;
        foreach (var now in new[] { deadline - 0.01f, deadline, deadline + 30f })
        {
            var firing = CombatMovementPolicyCore.AttackFiringPhaseActive(deadline, now);
            Equal(firing, InfantryCoverDecisionCore.ShouldHaltForCloseThreat(true, false, firing),
                "The early close-contact gate outlived its bounded firing phase.");
            var decision = InfantryCoverDecisionCore.EvaluateNeed(new CoverNeedInput(
                HasUsableCover: false, MayAdvanceFromCover: false,
                CoverCompromised: false, UnderDirectFire: false, Suppressed: false,
                CloseThreat: true, AttackAdvanceBlocked: true,
                NormalDecisionDue: false, UrgentDecisionDue: true,
                FiringPhaseActive: firing));
            Equal(!firing, decision.ShouldSearch,
                "Exposed close contact could not reach cover search after the initial response.");
            if (!firing)
                Equal(CoverSelectionMode.Urgent, decision.SelectionMode,
                    "Leaving open ground unnecessarily waited for an attack advance authorization.");
        }

        var blindCover = new CoverScoreInput(
            DistanceSqr: 64f, StandingPosePenalty: 0f, PrimaryThreatProtected: true,
            UnprotectedSecondaryThreats: 0, AssignedPoseCanFire: false,
            StandingCanFire: false, ExposedRouteMeters: 8f, ExposedRouteFraction: 1f);
        True(InfantryCoverDecisionCore.IsRouteAcceptable(CoverSelectionMode.Urgent, blindCover),
            "A protected destination required immediate LOS before leaving open ground.");
        False(InfantryCoverDecisionCore.IsRouteAcceptable(CoverSelectionMode.Urgent,
                blindCover with { PrimaryThreatProtected = false, AssignedPoseCanFire = true }),
            "An open firing position was accepted in place of actual protection.");
        True(InfantryCoverDecisionCore.ShouldLeaveCurrentCoverForAttackBound(
                new CoverPositionQuality(false, true, 0f, true),
                new CoverPositionQuality(true, true, 0.9f, false), forcedByDeadline: false),
            "An immediate firing lane justified remaining exposed instead of taking blind cover.");
    }

    private static void CloseCoverSearchKeepsSafetyAndRetryGates()
    {
        var exposed = new CoverNeedInput(
            HasUsableCover: false, MayAdvanceFromCover: false,
            CoverCompromised: false, UnderDirectFire: true, Suppressed: false,
            CloseThreat: true, AttackAdvanceBlocked: true,
            NormalDecisionDue: true, UrgentDecisionDue: true,
            FiringPhaseActive: false);
        False(InfantryCoverDecisionCore.EvaluateNeed(exposed with { Suppressed = true }).ShouldSearch,
            "The close-cover fix bypassed suppression safety.");
        False(InfantryCoverDecisionCore.EvaluateNeed(exposed with { HasUsableCover = true }).ShouldSearch,
            "An already-protected soldier abandoned cover just because the enemy was close.");
        var retry = InfantryCoverDecisionCore.EvaluateNeed(exposed with { UrgentDecisionDue = false });
        False(retry.ShouldSearch, "Close contact bypassed search cooldown or failed-route backoff.");
        Equal(InfantryCoverState.WaitingForSafeMove, retry.State,
            "A deferred close-cover search lost its waiting state.");
        True(InfantryCoverDecisionCore.EvaluateNeed(exposed).ShouldSearch,
            "An expired retry deadline did not release the close-cover search.");
    }

    private static void ContinuousCloseContactDoesNotRestartFiringDeadline()
    {
        var deadline = CombatMovementPolicyCore.EnsureInitialAttackFiringCommit(
            0f, false, true, true, 100f, CombatMovementPolicyCore.DefaultAttackFiringHoldSeconds);
        var now = deadline + 1f;
        ...
        False(ContactDivePolicyCore.ShouldStart(true, true, continuous, deadline, now),
            "Reacquiring an enemy during the cover move restarted the dive hold.");
        False(InfantryCoverDecisionCore.ShouldHaltForCloseThreat(true, false,
                CombatMovementPolicyCore.AttackFiringPhaseActive(deadline, now)),
            "The acquisition callback could re-halt a cover move after its firing deadline.");
    }

    private static void DefensiveAreaEntryDoesNotOwnAnEmptyCoverAssignment()
    {
        var areaOwned = DefensivePositionOwnershipCore.ShouldOwn(new DefensivePositionOwnershipInput(
            false, true, true, true, true));
        True(areaOwned, "The defender should still schedule defensive cover selection.");
        False(InfantryCoverDecisionCore.OwnsDefensiveCoverAssignment(areaOwned, false, false),
            "Area entry blocked native sandbag/building assignment before a slot was selected.");
        False(InfantryCoverDecisionCore.ShouldHoldDefensivePosition(areaOwned, false, false),
            "An empty cover inventory froze the defender at the objective boundary.");
        True(InfantryCoverDecisionCore.OwnsDefensiveCoverAssignment(areaOwned, true, false),
            "Native refresh could replace a reserved route through a doorway.");
        False(InfantryCoverDecisionCore.ShouldHoldDefensivePosition(areaOwned, true, false),
            "The defensive hold blocked transit to the actual cover slot.");
        True(InfantryCoverDecisionCore.OwnsDefensiveCoverAssignment(areaOwned, false, true),
            "Native refresh could replace an occupied defensive slot.");
        True(InfantryCoverDecisionCore.ShouldHoldDefensivePosition(areaOwned, false, true),
            "The defender did not hold once the selected slot was occupied.");
        False(InfantryCoverDecisionCore.ShouldHoldDefensivePosition(false, false, true),
            "A stale anchor retained movement after defensive ownership ended.");
    }

    private static void ProtectedUrbanCoverDoesNotRequireDistantLineOfSight()
    {
        var shelteredSlot = new CoverScoreInput(
            36f, 0f, true, 0, false, false, 4f, 0.67f,
            PrimaryProtectionFraction: 1f, PreferProtectionOverFiringLine: true);
        True(InfantryCoverDecisionCore.IsRouteAcceptable(CoverSelectionMode.DefensiveOccupation, shelteredSlot),
            "A wall beyond the cover slot vetoed an otherwise protective position.");
        True(InfantryCoverDecisionCore.CanAnchorDefensiveCover(true, shelteredSlot.PrimaryThreatProtected),
            "A protected slot without current enemy visibility could not become an anchor.");
        False(DefensiveAnchorReevaluationCore.ShouldReleaseForRealThreat(true, true, true, true),
            "A protected urban anchor was released merely because its firing lane was blocked.");
        var firingSlot = shelteredSlot with { AssignedPoseCanFire = true };
        True(InfantryCoverDecisionCore.Score(CoverSelectionMode.DefensiveOccupation, firingSlot) <
             InfantryCoverDecisionCore.Score(CoverSelectionMode.DefensiveOccupation, shelteredSlot),
            "Restoring indoor cover removed the preference for an equally protective firing position.");
        False(InfantryCoverDecisionCore.CanAnchorDefensiveCover(true, false),
            "Positively measured exposure was accepted as defensive protection.");
        False(InfantryCoverDecisionCore.CanAnchorDefensiveCover(false, true),
            "A failed evaluation incorrectly acquired a new anchor.");
    }

    private static void CloseContactCannotPermanentlyVetoCommittedUrbanMovement()
    {
        True(InfantryCoverDecisionCore.ShouldHaltForCloseThreat(true, false, true),
            "A new close rifle contact lost its initial firing halt.");
        False(InfantryCoverDecisionCore.ShouldHaltForCloseThreat(true, false, false),
            "Continuous close contact prevented a cover search or committed doorway/trench move.");
        False(InfantryCoverDecisionCore.ShouldHaltForCloseThreat(false, false, true),
            "The close-contact gate halted a distant engagement.");
        False(InfantryCoverDecisionCore.ShouldHaltForCloseThreat(true, true, true),
            "An eligible close-assault weapon could not keep its moving-fire permission.");
    }

    private static void CoverSelectionScoresEstimatedRouteInsteadOfVetoingEntrances()
    {
        ...
        True(InfantryCoverDecisionCore.IsRouteAcceptable(
            ...
            "Straight-chord exposure vetoed a protected destination before native entrance navigation.");
        var shelteredRoute = dangerousRoute with { ExposedRouteMeters = 0f, ExposedRouteFraction = 0f };
        True(InfantryCoverDecisionCore.Score(CoverSelectionMode.Normal, shelteredRoute) <
             InfantryCoverDecisionCore.Score(CoverSelectionMode.Normal, dangerousRoute),
            "Estimated exposure stopped influencing cover preference.");
        True(InfantryCoverDecisionCore.IsRouteAcceptable(
