...
internal static partial class ContactResponse
{
        ...
        float now)
    {
        ...
        if (!InfantryCoverDecisionCore.ShouldHaltForCloseThreat(
                closeThreat: true,
                HandheldWeaponClassifier.AllowsMovingFire(soldier, ai),
                IsContactDiveActive(state, now) ||
                CombatMovementPolicyCore.AttackFiringPhaseActive(state.AttackFiringCommitUntil, now)))
            return;
        if (state.Relocating)
    ...
    internal static bool MayWriteCoverAssignment(Soldier soldier)
    {
        ...
        return OwnsCoverAssignmentWrite(soldier) ||
               !InfantryCoverDecisionCore.OwnsDefensiveCoverAssignment(
                   ShouldControlTacticalPosition(soldier), state.Relocating,
