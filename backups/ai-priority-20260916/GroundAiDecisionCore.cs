...
internal static class HaltSpacingCore
{
    ...
    internal static bool HoldsOccupiedSlot(bool ownsReservation, bool relocating,
        float horizontalDistance, float verticalDistance)
        => ownsReservation && !relocating && float.IsFinite(horizontalDistance) &&
           float.IsFinite(verticalDistance) && horizontalDistance >= 0f && horizontalDistance <= 0.75f &&
           MathF.Abs(verticalDistance) <= 1.75f;

    internal static bool ShouldYieldToNeighbour(bool holdsOccupiedSlot,
        bool neighbourHoldsOccupiedSlot, bool neighbourCanYield,
        bool physicallyClipping, int soldierId, int neighbourId)
        // Preserve established cover against ordinary crowding. Two actual slot
        // occupants cannot both be exempt from resolving a physical overlap.
        => !holdsOccupiedSlot || physicallyClipping &&
           (!neighbourCanYield || neighbourHoldsOccupiedSlot && soldierId > neighbourId);

    internal static bool MayIgnoreDepartureBody(bool self, bool friendly, bool clippingAtStart)
        => self || friendly && clippingAtStart;
    ...
    /// passes) and his retry cooldown has elapsed. Callers can veto a protected slot;
    /// the runtime instead resolves protection against the actual neighbour after
    /// probing, so two physically overlapping slot owners cannot both be exempt.
    /// </summary>
...
internal readonly record struct DefensivePositionOwnershipInput(
    ...
    bool BoardingVehicle = false);

internal static class DefensivePositionOwnershipCore
{
    internal static bool ShouldOwn(DefensivePositionOwnershipInput input)
    {
        // A defender the game has sent to crew a vehicle is on his way to a seat, not
        // holding ground. Native HoldArea hands empty static guns out as covers, so
        // owning his position here halts him beside the gun on every update and undoes
        // the director's VehicleBoarding yield. Ownership returns once the order ends.
        if (!input.EligibleDefender ||
            !input.SquadFullySpawned ||
            input.ExternallyControlled ||
            input.BoardingVehicle)
            return false;
