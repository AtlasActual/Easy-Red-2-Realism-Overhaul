...
internal static class GroundAiDirector
{
        ...
        Squad? squad)
    {
        ...
        return new SoldierTacticalSnapshot(
            ...
            !mounted && HasNativeVehicleBoardingOrder(soldier));
    }

    /// <summary>
    /// True while the game has sent this on-foot soldier to a vehicle seat: a squad
    /// Get In order, or a static weapon that native HoldArea handed out as a cover.
    /// VehicleToOperate is null unless the soldier's destination is a vehicle, so the
    /// single native call also covers the no-destination case.
    /// </summary>
    internal static bool HasNativeVehicleBoardingOrder(Soldier soldier)
    {
        try
        {
