...
internal static partial class ContactResponse
{
        ...
        float now)
    {
        ...
        // A reservation may still be metres away. Probe actual bodies on the
        // bounded cadence; slot protection is resolved against the neighbour below.
        const bool holdsCoverSlot = false;
        ...
        float crowdRadius)
    {
        ...
        if (!CoverOccupancy.TryFindCrowdedFriendly(
                position, soldier, crowdRadius, out var neighbour, out var neighbourSoldier) ||
            neighbourSoldier == null)
            return false;

        var neighbourId = neighbourSoldier.GetInstanceID();
        var holdsSlot = HoldsPhysicalCoverSlot(soldier, state, soldierId, now);
        var neighbourHoldsSlot = AiState.ContactStates.TryGetValue(neighbourId, out var neighbourState) &&
                                 HoldsPhysicalCoverSlot(neighbourSoldier, neighbourState, neighbourId, now);
        var physicallyClipping = HorizontalDistanceSqr(position, neighbour) <=
                                 HaltSpacingCore.ClippingRadiusMeters * HaltSpacingCore.ClippingRadiusMeters;
        if (!HaltSpacingCore.ShouldYieldToNeighbour(holdsSlot, neighbourHoldsSlot,
                AiOwnership.IsAutonomous(neighbourSoldier), physicallyClipping, soldierId, neighbourId))
            return false;
        ...
        for (var pass = 0; pass < 2 && !foundTarget; pass++)
        {
                 ...
                 candidateIndex++)
            {
                ...
                var candidateTarget = new Vector3(
                    ...
                    position.y,
                    position.z + step.Z);
                if (!CoverRespectsDefensiveOrder(soldier, candidateTarget) ||
                    AiState.CoverReservedByOther(
                        IntPtr.Zero, candidateTarget, soldierId, now, passClearanceMeters) ||
                    AiState.HaltSpacingTargetReservedByOther(
        ...
        // Only give up an anchored slot once there is a reachable escape. A paused
        // cover run keeps its destination so the regular executor can resume it.
        if (!state.Relocating && state.ReservedCoverId != IntPtr.Zero)
            ReleaseContestedOccupiedCover(soldier, state, soldierId, state.ReservedCoverId, now);
    ...
    private static bool HoldsPhysicalCoverSlot(
        Soldier soldier, ContactResponseState state, int soldierId, float now)
        => HaltSpacingCore.HoldsOccupiedSlot(
            AiState.OwnsCoverReservation(state.ReservedCoverId, soldierId, now),
            state.Relocating,
            HorizontalDistance(soldier.transform.position, state.ReservedCoverPosition),
            soldier.transform.position.y - state.ReservedCoverPosition.y);
        ...
        try
        {
            ...
            foreach (var hit in hits)
            {
                if (hit.collider == null)
                    continue;
                var body = hit.collider.GetComponentInParent<Soldier>();
                if (body != null && HaltSpacingCore.MayIgnoreDepartureBody(
                        body.GetInstanceID() == soldier.GetInstanceID(),
                        string.Equals(body.faction, soldier.faction, StringComparison.Ordinal),
                        InfantryCoverDecisionCore.CoverPositionsConflict(
                            new MapPoint(from.x, from.z), from.y,
                            new MapPoint(body.transform.position.x, body.transform.position.z),
                            body.transform.position.y, HaltSpacingCore.ClippingRadiusMeters)))
                    continue;
                // Ignore only bodies already overlapping the departure point.
                // Walls and other bodies still block, and the endpoint must be empty.
                return false;
            }
